...
float kref[] = {0.0, 0.0, 0.0, 1.0, 1.0, 1.0};
...
three_doubles compute_shade(Vect start_p, Vect ray_direction, int depth)
{
	...
	if (min_ind < 3)
		...
		// spheres
		color.r = 255 * (min_ind == 3||min_ind == 4);
		color.g = 255 * (min_ind == 4);
		color.b = 255 * (min_ind == 5||min_ind == 4);
	...
	if(kref[min_ind] > 0)
	{
	    ...
	    reflected_color = compute_shade(intersect, ref_ray_direction, 1);
	    /*reflected_color.r /= 5;
        reflected_color.g /= 5;
        reflected_color.b /= 5;*/
    }
